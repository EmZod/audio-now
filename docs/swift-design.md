...
Rebuffer-after-stall (added 2026-07-18, field-driven): the fill threshold is its own atomic (`fillTarget`), reset to the small prebuffer at `beginJob` and raised *by the render thread* to a 2 s reserve the moment a mid-stream underrun fires. Frame-denominated refill (resume on the very next frame) turns a generation deficit into rapid-fire gap/audio/gap — 347 audible gaps in one measured session where the first job after idle ran at 0.4–1.0× realtime while macOS faulted compressed model pages back in. Seconds-denominated refill makes it one pause, then a smooth tail; end-of-stream overrides the hold so a job's last partial buffer always drains immediately.
